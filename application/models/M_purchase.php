...
class M_Purchase extends CI_Model {
    function query($type, $limit, $sort_by, $sort_order,$query, $offset, $option) {
        return;           // temp
...
//        if (strpos($option, 'hideitemdetail') !== FALSE) { $cond = $cond." AND tb_SalesBacklogItem.Status != 'completed'"; }
        
        $order_by = ($sort_by=="")? "":" ORDER BY $sort_by $sort_order, Item asc LIMIT $offset, $limit";

        $strSQL  = "SELECT *, tb_SalesBacklogItem.Note as ItemNote, tb_SalesBacklog.Note as OrderNote"
        ...
        $strSQL1 = "SELECT *, tb_SalesBacklogItem.Note as ItemNote, tb_SalesBacklog.Note as OrderNote"
                . " FROM tb_SalesBacklogItem "
                . "LEFT JOIN tb_SalesBacklog ON tb_SalesBacklog.ID = tb_SalesBacklogItem.BacklogID "
                . "LEFT JOIN tb_Company ON tb_SalesBacklogItem.CompanyID = tb_Company.ID "
    ...
    function get_single_purchase($id) {
